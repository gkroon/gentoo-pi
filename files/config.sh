...
new_user() {
  ...
  if [ -f "/root/.ssh/authorized_keys" ]; then
    if ! cp "/root/.ssh/authorized_keys" "/home/${NEW_USER}/.ssh/authorized_keys"; then
      printf "${FAILED}\n\nCould not copy /root/.ssh/authorized_keys to /home/${NEW_USER}/.ssh/authorized_keys. Exiting...\n"
      exit 1
    fi

    if ! chown ${NEW_USER}:${NEW_USER} "/home/${NEW_USER}/.ssh/authorized_keys"; then
    printf "${FAILED}\n\nCould not chown /home/${NEW_USER}/.ssh/authorized_keys. Exiting...\n"
    exit 1
    fi

    if ! chmod 0600 "/home/${NEW_USER}/.ssh/authorized_keys"; then
      printf "${FAILED}\n\nCould not chmod 0600 /home/${NEW_USER}/.ssh/authorized_keys. Exiting...\n"
      exit 1
    fi
  fi
}
...
printf '>>> Changing passwd for root\t' | expand -t 60
if passwd_root ; then
  printf "${OK}\n"
fi

printf '>>> Creating new user\t' | expand -t 60
if new_user ; then
  printf "${OK}\n"
fi

printf '>>> Changing passwd for new user\t' | expand -t 60
if new_user_passwd ; then
  printf "${OK}\n"
fi

printf '>>> Setting hostname\t' | expand -t 60
if setting_hostname ; then
  printf "${OK}\n"
fi

printf '>>> Enabling eth0 to start at boot\t' | expand -t 60
if enable_eth0 ; then
  printf "${OK}\n"
fi

printf '>>> Synchronising Portage\t' | expand -t 60
if sync_portage ; then
  printf "${OK}\n"
fi

if [[ "${HARDENED}" -eq "1" ]]; then
  printf '>>> Setting hardened profile\t' | expand -t 60
...
if [[ "${HARDENED}" -eq "1" ]]; then
  printf '>>> Rebuilding GCC (could take a few hours)\t' | expand -t 60
...
printf '>>> Updating needed packages (could take a few hours)\t' | expand -t 60
if update_packages ; then
  printf "${OK}\n"
fi

printf '>>> Installing needed packages (could take a few hours)\t' | expand -t 60
if install_packages ; then
  printf "${OK}\n"
fi

printf '>>> Configuring packages\t' | expand -t 60
if configure_packages ; then
  printf "${OK}\n"
fi

printf '>>> Compiling kernel (could take a few hours )\t' | expand -t 60
if compile_kernel ; then
  printf "${OK}\n"
fi

printf '>>> Enabling services\t' | expand -t 60
